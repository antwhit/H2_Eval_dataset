 ...
 * @version CVS $Revision: 1.1 $ $Date: 2002/01/19 14:19:06 $
 */
public final class XConfTool extends Task {
     ...
     * Scan recursive
     */
    private void process(final File directoryFile, final String ext, final String configurationLocation) throws IOException, BuildException {
     ...
     */
    private void add(final String configurationLocation, final String newComponent) throws IOException {
        ...
        if (data.indexOf(newComponent) == -1) {
            ...
            if (pos != -1) {
                ...
                if (pos != -1) {
                    StringBuffer buffer = new StringBuffer(data.substring(0, pos + 1)).append("\n\n").append(newComponent).append(data.substring(pos + 1));
