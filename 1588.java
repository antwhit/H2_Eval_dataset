...
public class CountOpcodes {
    ...
    public static void handleBytecode(BytecodeInfo bc) {
        for (Iterator i = bc.getInstructions().iterator(); i.hasNext(); ) {
            ...
            if (p == null) predsCount[0]++; else predsCount[p.length]++;
            Instruction[] s = instr.getSuccs();
            if (s == null) succsCount[0]++; else succsCount[s.length]++;
        }
    }
    ...
    public static void handlePackage(String pack) {
        ...
        while (subs.hasMoreElements()) {
            ...
            if (ClassInfo.isPackage(full)) handlePackage(full); else {
    ...
    public static void main(String[] params) {
        ...
        for (int i = 0; i < 256; i++) {
            if (opcodeCount[i] > 0) System.err.println("Opcode " + i + ": \t   (" + Opcodes.opcodeString[i] + ")\t" + opcodeCount[i]);
        }
        int moreThanTwo = 0;
        for (int i = 0; i < predsCount.length; i++) {
            if (predsCount[i] > 0) {
                System.err.println("preds " + i + ": \t" + predsCount[i]);
                if (i > 1) moreThanTwo += predsCount[i];
            }
        }
        ...
        for (int i = 0; i < succsCount.length; i++) {
            if (succsCount[i] > 0) {
                ...
                if (i > 1) moreThanTwo += succsCount[i];
            }
        }
