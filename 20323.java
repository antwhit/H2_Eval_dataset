...
public class T6942649 {
    ...
    void test(String opt, String clazz) throws Exception {
        ...
        if (!out.contains(clazz)) throw new Exception("class name not found in output");
        int lastDot = clazz.lastIndexOf(".");
        if (!out.contains(clazz.substring(lastDot + 1) + ".class")) throw new Exception("location of class not found in output");
        if (!out.contains("MD5 checksum: ")) throw new Exception("checksum not found in output");
    }
}
