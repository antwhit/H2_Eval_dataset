...
public class T6394683 {
    ...
    abstract static class TestFile extends File {
    ...
    static TestFile good_class = new TestFile(b_class) {
        ...
        void create() throws IOException {
            ...
            if (rc != 0) throw new AssertionError("compilation failed, rc=" + rc + " creating B.class");
        }
    };
    ...
    public static void main(String... args) throws Exception {
        ...
        if (!ok) throw new AssertionError("test failed");
    }
    ...
    static boolean test(String opt, TestFile older, TestFile newer) throws Exception {
        ...
        older.create();
        newer.create();
        if (!older.exists() || !newer.exists()) throw new AssertionError("error creating files");
        int n = 0;
        while (newer.lastModified() <= older.lastModified()) {
            if (++n == 5) throw new Error("Cannot create files");
            Thread.sleep(1000);
            newer.create();
        }
        System.err.println("test:" + "option:" + opt + ", " + "older:" + older + "[" + older.length() + ":" + older.lastModified() + "], " + "newer:" + newer + "[" + newer.length() + ":" + newer.lastModified() + "]");
        for (String s : new File(".").list()) System.err.print(" " + s);
        System.err.println();
        JavaCompiler javac = ToolProvider.getSystemJavaCompiler();
        int rc = javac.run(null, null, null, "-d", ".", "-classpath", ".", "-sourcepath", ".", opt, a_java.getPath());
        if (rc != 0) {
