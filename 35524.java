...
public class edgetracker extends PApplet {
    ...
    public void draw() {
        ...
        if (frameCount % 5 == 0) sendOsc(video.pixels);
    }

    public float diff(int p, int off) {
        if (p + off < 0 || p + off >= numPixels) return 0;
        return red(video.pixels[p + off]) - red(video.pixels[p]) + green(video.pixels[p + off]) - green(video.pixels[p]) + blue(video.pixels[p + off]) - blue(video.pixels[p]);
    }
