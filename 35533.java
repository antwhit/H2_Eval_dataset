...
public class T7018098 extends JavacTestingAbstractProcessor {
    ...
    void run() throws Exception {
        ...
        compile("-proc:only", "-processor", myName, "-Aexpect=false", file.getPath());
        testDir.mkdirs();
        _assert(testDir.exists());
        compile("-proc:only", "-processor", myName, "-Aexpect=true", file.getPath());
    }
    ...
    <T> void checkEqual(String label, T actual, T expected) {
        if (actual != expected) messager.printMessage(Diagnostic.Kind.ERROR, "Unexpected value for " + label + "; expected: " + expected + "; found: " + actual);
    }
