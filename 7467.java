 ...
 */
final class OPT_DepGraphNode extends OPT_SpaceEffGraphNode implements OPT_DepGraphConstants {
   ...
   */
    public void insertOutEdge(OPT_DepGraphNode node, int type) {
        if (COMPACT) {
            int numTries = 0;
            for (OPT_DepGraphEdge oe = (OPT_DepGraphEdge) firstOutEdge(); oe != null && numTries < 4; oe = (OPT_DepGraphEdge) oe.getNextOut(), numTries++) {
                if (oe.toNode() == node) {
                    oe.addDepType(type);
                    return;
                }
