public class BooleanIndividual extends Individual {
    ...
    public String toString() {
        String ret = "";
        for (int i = 0; i < chromosome.length; i++) ret += chromosome[i] ? "T" : "F";
        return ret;
    }

    public int getFitness() {
        int health = 0;
        for (int i = 0; i < chromosome.length; i++) if (chromosome[i]) health++;
    ...
    public Individual[] crossWith(Individual mom) {
        ...
        if (len > momsDNA.length) len = momsDNA.length;
        boolean temp;
    ...
    public Individual getMutation() {
        ...
        for (int i = 0; i < DNA.length; i++) DNA[i] = (Math.random() < .10) ? !DNA[i] : DNA[i];
