...
public class AuctionSimulator implements CountDownTimerListener, ActionListener {
    ...
    private void start() {
        if (closed) frame.setMsg("Auction closed."); else {
    ...
    private void close() {
        ...
        if (winner == null) frame.setMsg("Closed with no bidders."); else if (winner == myPos) frame.setMsg("You win!!"); else frame.setMsg("Auction closed.");
    }
    ...
    public void timerTicked(CountDownTimerEvent e) {
        ...
        if (timeLeft == 0) close(); else if (timeLeft % 5 == 0) newBid();
    }
    ...
    private void newBid() {
        ...
        if (choice == size) newPos = bid(null, "", bidAmount); else {
    ...
    private void myBid() {
        ...
        if (myPos != null) frame.withdrawBtn.setEnabled(true);
    }
