...
public class Session extends ERXSession {
    ...
    public void awake() {
        super.awake();
        if (user() != null) User.setActor(user());
    }
    ...
    public String loginUser(String username, String password) {
        ...
        if (username != null && password != null && username.length() > 0 && password.length() > 0) {
            ...
            if (user() == null) {
                ...
                if (potentialUsers.count() > 1) log.warn("Found multiple Users for the username: " + username + " found users: " + potentialUsers);
            }
        } else {
    ...
    public NSArray groups() {
        return EOUtilities.objectsForEntityNamed(defaultEditingContext(), "Group");
    }

    public NSArray users() {
        return EOUtilities.objectsForEntityNamed(defaultEditingContext(), "User");
    }

    public EOArrayDataSource usersDataSource() {
        return ERXUtilities.dataSourceForArray(users());
    }

    public EOArrayDataSource groupsDataSource() {
        return ERXUtilities.dataSourceForArray(groups());
    }
}
