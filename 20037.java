...
public class LTP {
    ...
    public void manning() {
        if (this.ltpSet == false && quality > 0) {
            ...
            ltpEastingPerMinute = ((float) Math.cos((((float) this.latitudeMinuteInt) + this.latitudeMinuteFract) * 0.00029089f)) * metresPerMinute;
            this.ltpSet = true;
        }
        if (this.ltpSet && quality > 0) {
            float latMinDiff = (float) (latitudeMinuteInt - ltpLatMinuteInt) + (latitudeMinuteFract - ltpLatMinuteFract);
            float lonMinDiff = (float) (longitudeMinuteInt - ltpLonMinuteInt) + (longitudeMinuteFract - ltpLonMinuteFract);
