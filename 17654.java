...
public class T6996914a {
    ...
    enum ConstructorKind {
        ...
        PACKAGE(""), PROTECTED("protected"), PRIVATE("private"), PUBLIC("public");
    ...
    static class FooClass extends SimpleJavaFileObject {
        ...
        static final String sourceStub = "#P\n" + "public class Foo<X> {\n" + "  #M Foo() {}\n" + "}\n";

        String source;

        public FooClass(PackageKind pk, ConstructorKind ck) {
            super(URI.create("myfo:/" + (pk != PackageKind.DEFAULT ? "a/Foo.java" : "Foo.java")), JavaFileObject.Kind.SOURCE);
    ...
    static class ClientClass extends SimpleJavaFileObject {
        ...
        static final String sourceStub = "#I\n" + "class Test {\n" + "  Foo<String> fs = new Foo<>();\n" + "}\n";
    ...
    static boolean check(PackageKind pk, ConstructorKind ck) {
        switch(pk) {
            case A:
                return ck == ConstructorKind.PUBLIC;
            case DEFAULT:
                return ck != ConstructorKind.PRIVATE;
            default:
                throw new AssertionError("Unknown package kind");
        }
    }
