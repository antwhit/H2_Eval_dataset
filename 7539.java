...
public class T6440583 {
    ...
    public static void main(String... args) throws Exception {
        ...
        Iterable<? extends JavaFileObject> files = fm.getJavaFileObjectsFromFiles(Arrays.asList(new File(testSrc, "A.java")));
        ...
        TreeScanner<Boolean, Void> checker = new TreeScanner<Boolean, Void>() {
            ...
            public Boolean visitErroneous(ErroneousTree tree, Void ignore) {
                ...
                if (errs == null || errs.size() == 0) throw new AssertionError("no error trees found");
                found = true;
                return true;
            }
        };
        for (Tree tree : trees) checker.scan(tree, null);
        if (!found) throw new AssertionError("no ErroneousTree nodes found");
    }
