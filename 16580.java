...
public class PacketLogger {
    ...
    public void log(byte[] packet) {
        ...
        while (pos < packet.length) {
            ...
            short startpos = pos;
            pos += 16;
            if (pos > packet.length) pos = (short) packet.length;
            for (short i = startpos; i < pos; i++) {
                out.print(hex(packet[i]) + " ");
            }
            for (short i = pos; i < startpos + 16; i++) out.print("   ");
            out.print("    ");
            for (short i = startpos; i < startpos + 16; i++) {
                if (i >= pos) out.print(" "); else if (packet[i] < 32) out.print("."); else out.print((char) packet[i]);
            }
            out.println("");
        }
        out.println("");
