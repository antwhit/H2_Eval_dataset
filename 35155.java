...
public final class Configuration extends Object implements WOXMLCoding {
    ...
    private NSArray availableFontLists = new NSArray(new Object[] { "Arial,Helvetica", "Osaka,MS P Gothic" });
    ...
    public void removePath(String path) {
        if (paths.containsObject(path)) paths.removeObject(path);
    }

    public void removeRule(GSVRule rule) {
        if (quickRules.containsObject(rule)) quickRules.removeObject(rule);
    }

    public void addRule(GSVRule rule) {
        if (!quickRules.containsObject(rule)) quickRules.addObject(rule);
    }
    ...
    public Configuration(WOXMLDecoder decoder) {
        ...
        if (fontList == null || fontList.length() == 0) fontList = (String) availableFontLists.objectAtIndex(0);
    }
