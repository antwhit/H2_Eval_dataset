...
public class T6468404 extends ToolTester {
    ...
    void test(String... args) {
        ...
        task = tool.getTask(null, fm, null, null, null, Collections.singleton(new DummyFO("C")));
        task.setProcessors(Collections.singleton(new P()));
        if (!task.call()) throw new AssertionError();
        System.err.println("Compiling with binaries w/o -g:");
        task = tool.getTask(null, fm, null, null, null, Collections.singleton(new DummyFO("Dummy")));
        task.setProcessors(Collections.singleton(new P()));
        if (!task.call()) throw new AssertionError();
        task = tool.getTask(null, fm, null, Arrays.asList("-g"), null, Collections.singleton(new DummyFO("C")));
        if (!task.call()) throw new AssertionError();
...
class P extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        if (!ran) {
            ...
            if (!isParameterized(type)) throw new AssertionError(type);
