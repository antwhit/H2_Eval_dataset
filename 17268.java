...
class OPT_DepGraphStats implements OPT_Operators {
   ...
   */
    OPT_DepGraphStats(OPT_DepGraph dg, String bbName) {
        ...
        for (OPT_DepGraphNode n = (OPT_DepGraphNode) dg.firstNode(); n != null; n = (OPT_DepGraphNode) n.getNext()) {
            ...
            if (instr.isImplicitStore() || instr.isImplicitLoad()) containsLoadOrStore = true;
        }
        OPT_DepGraphNode nodes[] = new OPT_DepGraphNode[numNodes];
        int ECT[] = new int[numNodes];
        int totalTime = 0;
        ...
        System.out.println("@@@@ BB " + bbName + "; totalTime = " + totalTime + "; containsLoadOrStore = " + containsLoadOrStore + "; critPathLength = " + critPathLength);
    }
   ...
   */
    static void printBasicBlockStatistics(OPT_IR ir) {
        ...
        for (OPT_BasicBlock bb = ir.firstBasicBlockInCodeOrder(); bb != null; bb = bb.nextBasicBlockInCodeOrder()) {
            OPT_DepGraph dg = new OPT_DepGraph(ir, bb.firstRealInstruction(), bb.lastRealInstruction(), bb);
   ...
   */
    int estimateExecutionTime(OPT_Instruction instr) {
        if (instr.operator() == NULL_CHECK) return 0; else return 1;
    }
}
