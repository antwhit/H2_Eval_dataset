...
class VM_StackBrowser implements VM_Constants {
    ...
    private void upOneFrame() {
        ...
        if (!hasMoreFrames()) throw new Error("Error during stack browsing");
        int cmid = VM_Magic.getCompiledMethodID(newFP);
        VM_CompiledMethod cm = VM_CompiledMethods.getCompiledMethod(cmid);
        while (cmid == INVISIBLE_METHOD_ID) {
            if (cm.getMethod().getDeclaringClass().isBridgeFromNative()) newFP = VM_Runtime.unwindNativeStackFrame(newFP); else newFP = VM_Magic.getCallerFramePointer(newFP);
