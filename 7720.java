...
public class Node {
    ...
    protected int in_edges[];

    protected int out_edges[];
    ...
    public Node(Node nd) {
        ...
        weight = nd.weight;
        layer = nd.layer;
        if (nd.in_edges == null) in_edges = null; else if (nd.in_edges.length == 0) in_edges = null; else {
            in_edges = new int[nd.in_edges.length];
            System.arraycopy(nd.in_edges, 0, in_edges, 0, nd.in_edges.length);
        }
        if (nd.out_edges == null) out_edges = null; else if (nd.out_edges.length == 0) out_edges = null; else {
    ...
    private String getFileText(String dir, String body) {
        ...
        try {
            ...
            while (input.ready()) result += input.readLine() + "\n";
            input.close();
            return result;
