    ...
    public String parseAndExecuteCommand(String line) throws CommunicationException, Exception {
        ...
        if (line != null) {
            if (line.length() == 0) {
                ...
                } else strbuf.append("Session already open" + CR);
            } else if (line.equals("close")) {
                if (comm.isOpen()) {
                    comm.close();
                    strbuf.append("Session closed" + CR);
                } else strbuf.append("No session currently open" + CR);
            } else {
                ...
                if (msg != null) {
                    if (comm.isOpen()) {
                        ...
                        if (reply != null) strbuf.append(reply + CR); else strbuf.append("Invalid reply" + CR);
                    } else strbuf.append("No session currently open" + CR);
                } else strbuf.append(output);
            }
        }
