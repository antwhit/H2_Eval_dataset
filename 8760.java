...
class VM_ApplicationClassLoader extends AppClassLoader {
    ...
    public static void setPathProperty() {
        ...
        if (repositories != null) for (int i = 0, n = repositories.length; i < n; ++i) {
            String name = repositories[i];
            if (name.startsWith(".")) name = System.getProperty("user.dir") + File.separator + name.substring(1);
            if (classpath == null) classpath = name; else classpath += File.pathSeparator + name;
        }
    ...
    private String findRepository(String resourceName) {
        for (int i = 0; i < parsedPath.length; i++) if (resourceName.startsWith(parsedPath[i])) return parsedPath[i]; else if (resourceName.startsWith(toURLString(parsedPath[i]))) return parsedPath[i];
