...
import javax.tools.*;
import pl.wcislo.sbql4j.source.util.*;
import pl.wcislo.sbql4j.tools.javac.api.*;
import pl.wcislo.sbql4j.tools.javac.main.*;
import pl.wcislo.sbql4j.tools.javac.util.*;
...
public class T6358024 extends AbstractProcessor {
    ...
    public static void main(String... args) throws Throwable {
        ...
        test(fm, f, new Option[] { new XOption("-XprintRounds"), new Option("-processorpath", "."), new Option("-processor", self) }, 11);
    }
    ...
    static void test(JavacFileManager fm, JavaFileObject f, Option[] opts, int expect) throws Throwable {
        ...
        for (Option opt : opts) {
            flags.add(opt.name);
            for (Object arg : opt.args) flags.add(arg.toString());
        }
        JavacTaskImpl task = (JavacTaskImpl) tool.getTask(out, fm, null, flags, null, Arrays.asList(f));
        MyTaskListener tl = new MyTaskListener();
        task.setTaskListener(tl);
        task.call();
        if (tl.started != expect) throw new AssertionError("Unexpected number of TaskListener events; " + "expected " + expect + ", found " + tl.started);
    }
