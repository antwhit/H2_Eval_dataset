...
public class PmeAlgSF extends Applet implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand() == facButton.getLabel()) {
            ...
            if (!(3 < n1.intValue() && n1.intValue() < 64000) || !(2 < B1.intValue() && B1.intValue() < 64000) || !(1 < a1.intValue() && a1.intValue() < n1.intValue())) ResultTA.setText("No valid input.\n" + "The values have to be:\n3 < n < 64000\n2" + " < B < 64000\n1 < a < n"); else {
                Faktor = new PmeAlg(n1.intValue(), B1.intValue(), a1.intValue());
                showRes();
            }
        }
    ...
    private void showRes() {
        ...
        for (int i = 0; i != s; i++) ResultTA.append("\t" + Faktor.qList.elementAt(i) + "\t" + Faktor.lList.elementAt(i) + "\t" + Faktor.aList.elementAt(i) + "\n");
        if (Faktor.success == true) {
            Result.setText("Factorization succeeded");
            if (Faktor.d == Faktor.firstd) ResultTA.append("\nfound non-trivial factor d =\t" + Faktor.d + "\nanother non-trivial factor:\t" + (Faktor.n / Faktor.d)); else {
                ResultTA.append("\nfound non-trivial factor:\nd = gcd( " + Faktor.aList.elementAt(s - 1) + " - 1, n) = " + Faktor.d + "\nanother non-trivial factor is:\t" + (Faktor.n / Faktor.d));
            }
        } else {
