...
public class ClassTreeTest {
    ...
    void run() throws Exception {
        ...
        int expected = 4;
        if (checks != expected) error("Unexpected number of checks performed; expected: " + expected + ", found: " + checks);
    ...
    void check(CompilationUnitTree cu, String name, Tree.Kind k) {
        ...
        TreeScanner<ClassTree, String> s = new TreeScanner<ClassTree, String>() {
            ...
            public ClassTree visitClass(ClassTree c, String name) {
                if (c.getSimpleName().toString().equals(name)) return c; else return super.visitClass(c, name);
            }
        ...
        if (c == null) error("Can't find node named " + name); else if (c.getKind() != k) error("Unexpected kind for node named " + name + ": expected: " + k + ", found: " + c.getKind());
    }
