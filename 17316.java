...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (JavaFileObject file : fm.list(PLATFORM_CLASS_PATH, "", EnumSet.of(CLASS), true)) {
            ...
            if (type.endsWith("package-info")) continue;
            try {
                ...
                if (type != null) System.err.println("Looking at " + type);
            }
        }
        javac = null;
        elements = null;
        ...
        if (classes < 9000) throw new AssertionError("Too few classes in PLATFORM_CLASS_PATH ;-)");
        if (packages.size() < 530) throw new AssertionError("Too few packages in PLATFORM_CLASS_PATH ;-)");
        if (nestedClasses < 3000) throw new AssertionError("Too few nested classes in PLATFORM_CLASS_PATH ;-)");
    }
}
