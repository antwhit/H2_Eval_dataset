...
public class dig {
    ...
    static void doAXFR(Message query, Resolver res) throws IOException {
        Message response;
    ...
    public static void main(String argv[]) throws IOException {
        ...
        try {
            ...
            if (nameString.equals("-x")) {
                ...
                if (type < 0) type = Type.A; else arg++;
                _class = DClass.value(argv[arg]);
                if (_class < 0) _class = DClass.IN; else arg++;
            }
            while (argv[arg].startsWith("-") && argv[arg].length() > 1) {
                switch(argv[arg].charAt(1)) {
                    case 'p':
                        String portStr;
                        int port;
                        if (argv[arg].length() > 2) portStr = argv[arg].substring(2); else portStr = argv[++arg];
                    ...
                    case 'k':
                        String key;
                        if (argv[arg].length() > 2) key = argv[arg].substring(2); else key = argv[++arg];
                        int index = key.indexOf('/');
                        if (index < 0) res.setTSIGKey(key); else res.setTSIGKey(key.substring(0, index), key.substring(index + 1));
                        break;
                    case 't':
                        res.setTCP(true);
                        break;
        ...
        if (type == Type.AXFR) doAXFR(query, res); else doQuery(query, res);
    }
}
