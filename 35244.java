...
public class dnsRecord {
    ...
    static dnsRecord newRecord(dnsName name, short type, short _class) {
        switch(type) {
            case dns.A:
                return new dnsARecord(name, _class);
            case dns.NS:
                return new dnsNSRecord(name, _class);
            case dns.CNAME:
                return new dnsCNAMERecord(name, _class);
            case dns.SOA:
                return new dnsSOARecord(name, _class);
            case dns.PTR:
                return new dnsPTRRecord(name, _class);
            case dns.MX:
                return new dnsMXRecord(name, _class);
            case dns.TXT:
                return new dnsTXTRecord(name, _class);
            case dns.KEY:
                return new dnsKEYRecord(name, _class);
            case dns.SIG:
                return new dnsSIGRecord(name, _class);
            case dns.NXT:
                return new dnsNXTRecord(name, _class);
            case dns.TSIG:
                return new dnsTSIGRecord(name, _class);
            default:
                return new dnsRecord(name, type, _class);
        }
    }

    static dnsRecord buildRecord(CountedDataInputStream in, int section, dnsCompression c) throws IOException {
        short type, _class;
        dnsName name;
        dnsRecord rec;
        ...
        if (section == dns.QUESTION) return rec;
        rec.rttl = in.readInt();
    ...
    void toBytes(DataOutputStream out, int section) throws IOException {
        ...
        if (section == dns.QUESTION) return;
        out.writeInt(rttl);
    ...
    public String toString() {
        ...
        if (rr != null) {
            if (this instanceof dnsSIGRecord) sb.append(" "); else sb.append("\t");
            sb.append(rr);
        }
        return sb.toString();
    }
