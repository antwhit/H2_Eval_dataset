...
public class Weblogs {
    ...
    static List listChannels() throws IOException, XmlPullParserException {
        return listChannels("http://static.userland.com/weblogMonitor/logs.xml");
    }

    static List listChannels(String uri) throws IOException, XmlPullParserException {
        ...
        while (parser.nextTag() == parser.START_TAG) {
            ...
            if (url != null) result.addElement(url);
        }
    ...
    public static String readSingle(XmlPullParser parser) throws IOException, XmlPullParserException {
        String url = null;
        parser.require(parser.START_TAG, "", "log");
        while (parser.nextTag() == parser.START_TAG) {
            String name = parser.getName();
            String content = parser.nextText();
            if (name.equals("url")) url = content;
    ...
    public static void main(String[] args) throws IOException, XmlPullParserException {
        List urls = args.length > 0 ? listChannels(args[0]) : listChannels();
        for (Iterator i = urls.iterator(); i.hasNext(); ) System.out.println(i.next());
    }
}
