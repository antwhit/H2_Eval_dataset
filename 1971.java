...
public class T6917288 {
    ...
    int compile(List<String> opts, File... files) {
        ...
        if (out.length() > 0) System.err.println(out);
        return rc;
    }
    ...
    void check(File dir, String... paths) {
        ...
        if (found.equals(expect)) return;
        for (String f : found) {
            if (!expect.contains(f)) error("Unexpected file found: " + f);
        }
        for (String e : expect) {
            if (!found.contains(e)) error("Expected file not found: " + e);
        }
    }
