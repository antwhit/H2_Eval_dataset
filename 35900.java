...
public class java_lib_various_runme {
    ...
    public static void main(String argv[]) {
        ...
        if (java_lib_various.check_animals(animals) != 1) throw new RuntimeException("check_animals failed");
        String expected[] = { "Dave", "Mike", "Susan", "John", "Michelle" };
        String got[] = java_lib_various.get_names();
        for (int i = 0; i < got.length; i++) if (!got[i].equals(expected[i])) throw new RuntimeException("Name failed " + i + " " + got[i] + "|" + expected[i]);
        String langscheck[] = { "Hungarian", "Afrikaans", "Norwegian" };
        String langs[] = java_lib_various.getLanguages();
        for (int i = 0; i < langs.length; i++) if (!langs[i].equals(langscheck[i])) throw new RuntimeException("Languages read failed " + i + " " + langs[i] + "|" + langscheck[i]);
        String newLangs[] = { "French", "Italian", "Spanish" };
        java_lib_various.setLanguages(newLangs);
        langs = java_lib_various.getLanguages();
        for (int i = 0; i < langs.length; i++) if (!langs[i].equals(newLangs[i])) throw new RuntimeException("Languages verify failed " + i + " " + langs[i] + "|" + newLangs[i]);
        {
            String stringOutArray[] = { "" };
            java_lib_various.char_ptr_ptr_out(stringOutArray);
            if (!stringOutArray[0].equals("returned string")) throw new RuntimeException("Test failed: expected: returned string. got: " + stringOutArray[0]);
        }
        try {
        ...
        for (int i = 0; i < byjovestring.length(); i++) {
            if (byjove[i] != b[i]) throw new RuntimeException("By jove, it failed: [" + new String(b) + "]");
        }
    }
}
