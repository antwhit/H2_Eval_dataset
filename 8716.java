class Globals {
    ...
    public static int MaxDisks = 64;
}

public class hanoioo {

    static Peg peg1 = new Peg(1), peg2 = new Peg(2), peg3 = new Peg(3);

    public static void main(String[] args) {
        Globals.NumDisks = 24;
        if (args.length > 0) Globals.NumDisks = Integer.parseInt(args[0]);
        System.out.println("moving " + Globals.NumDisks + " disks...");
        if (Globals.NumDisks > Globals.MaxDisks) Globals.NumDisks = Globals.MaxDisks;
        for (int i = Globals.NumDisks; i > 0; i--) peg1.addDisk(i);
...
class Peg {
    ...
    public Peg(int n) {
        pegNum = n;
        for (int i = 0; i < Globals.NumDisks; i++) disks[i] = 0;
        nDisks = 0;
    }
