...
public class SearchNGramsServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("xml".equals(output)) response.setContentType("text/xml"); else response.setContentType("text/plain");
        if (query == null || query.length() == 0) {
            ...
            if ("xml".equals(output)) response.getWriter().write("<ngram-frequency><error>true</error><message>Please specify the query ngram using the \"q\" parameter</message></ngram-frequency>"); else response.getWriter().write("Please specify the query ngram using the \"q\" parameter");
        } else {
