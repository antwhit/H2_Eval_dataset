...
public class PrintButtonListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        while (sayac < exactMatchList.size()) {
            ...
            for (int i = 0; i < Printer.NAMELENGTH - exactMatchList.get(sayac).getText().length(); i++) outputString += " ";
            outputString += exactMatchList.get(sayac + 1).getText();
            for (int i = 0; i < Printer.PATHLENGTH - exactMatchList.get(sayac + 1).getText().length(); i++) outputString += " ";
            outputString += exactMatchList.get(sayac + 2).getText();
            outputString += "\n";
            if (MainFrame.DEBUG) System.out.println("yazilacak satir :\"" + outputString + "\"");
            sayac += 3;
        }
        sayac = 0;
        if (likeMatchList.size() > 0) {
            ...
            while (sayac < likeMatchList.size()) {
                ...
                for (int i = 0; i < 7 - likeMatchList.get(sayac).getText().length(); i++) outputString += " ";
                outputString += likeMatchList.get(sayac + 1).getText();
                for (int i = 0; i < Printer.PATHLENGTH - likeMatchList.get(sayac + 1).getText().length(); i++) outputString += " ";
                outputString += likeMatchList.get(sayac + 2).getText();
                outputString += "\n";
                if (MainFrame.DEBUG) System.out.println("yazilacak yaklasik satir \"" + outputString + "\"");
                sayac += 3;
            }
        }
