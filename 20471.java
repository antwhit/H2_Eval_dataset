...
final class OPT_ResourceMap {
   ...
   */
    ...
    private void grow(int s) {
        if (verbose >= 2) debug("Growing from " + size + " to " + s);
        if (size >= s) return;
        int len = rumap.length;
        if (len < s) {
            for (; len < s; len <<= 1) ;
            int[] t = new int[len];
            for (int i = 0; i < rumap.length; i++) t[i] = rumap[i];
            for (int i = rumap.length; i < len; i++) t[i] = OPT_OperatorClass.NONE;
            rumap = t;
        }
        size = s;
    }
   ...
   */
    public OPT_ResourceMap(int length) {
        ...
        size = 0;
        for (int i = 0; i < length; i++) rumap[i] = OPT_OperatorClass.NONE;
    }
   ...
   */
    public boolean schedule(OPT_Instruction i, int time) {
        if (OPT_SchedulingInfo.isScheduled(i)) throw new InternalError("Already scheduled");
        OPT_OperatorClass opc = i.operator().getOpClass();
        if (verbose >= 2) debug("Op Class=" + opc);
   ...
   */
    public void unschedule(OPT_Instruction i) {
        if (!OPT_SchedulingInfo.isScheduled(i)) throw new InternalError("Not scheduled");
        OPT_OperatorClass opc = i.operator().getOpClass();
   ...
   */
    ...
    private boolean schedule(int[] usage, int time) {
        ...
        if (verbose >= 1) {
            ...
            for (int i = 0; i < usage.length; i++) debug("   " + toBinaryPad32(usage[i]));
            debug("");
        }
        for (int i = 0; i < usage.length; i++) if ((usage[i] & rumap[time + i]) != 0) return false;
