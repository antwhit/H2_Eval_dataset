...
public class SplitLongLines {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length == 0) System.err.println("Using default argument of \"" + sourceUrlString + '"'); else sourceUrlString = args[0];
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                for (Tag tag : source.getAllTags()) {
                    if (pos != tag.getBegin()) print(line.subSequence(pos, tag.getBegin()));
                    printTag(tag, line);
                    pos = tag.getEnd();
                }
                if (pos != line.length()) print(line.subSequence(pos, line.length()));
                println();
            }
        } finally {
