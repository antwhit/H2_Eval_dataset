...
public class ErrorListPlugin extends EBPlugin {
    ...
    public void handleMessage(EBMessage message) {
        if (message instanceof ErrorSourceUpdate) handleErrorSourceMessage((ErrorSourceUpdate) message); else if (message instanceof ViewUpdate) handleViewMessage((ViewUpdate) message); else if (message instanceof EditPaneUpdate) handleEditPaneMessage((EditPaneUpdate) message); else if (message instanceof PropertiesChanged) propertiesChanged(); else if (message instanceof CreateDockableWindow) handleCreateDockableMessage((CreateDockableWindow) message);
    }
    ...
    private void showErrorList(View view) {
        ...
        if (!dockableWindowManager.isDockableWindowVisible(NAME)) dockableWindowManager.addDockableWindow(NAME);
    }
    ...
    private void handleErrorSourceMessage(ErrorSourceUpdate message) {
        ...
        if (what == ErrorSourceUpdate.ERROR_ADDED || what == ErrorSourceUpdate.ERROR_REMOVED) {
            if (showOnError) {
                ...
                if (view == null) showOnStartup = true; else showErrorList(view);
            }
            ...
            while (view != null) {
                if (view.getBuffer() == buffer) {
                    ...
                    for (int i = 0; i < editPanes.length; i++) editPanes[i].getTextArea().invalidateLine(lineNumber);
                }
                view = view.getNext();
            }
            ...
            while (view != null) {
                ...
                for (int i = 0; i < editPanes.length; i++) {
                    editPanes[i].getTextArea().getPainter().repaint();
                }
                view = view.getNext();
            }
        }
    ...
    private void handleEditPaneMessage(EditPaneUpdate message) {
        if (message.getWhat() == EditPaneUpdate.CREATED) {
            ...
            message.getEditPane().getTextArea().getPainter().addCustomHighlight(highlight);
        }
    }

    private void handleCreateDockableMessage(CreateDockableWindow message) {
        if (message.getDockableWindowName().equals(NAME)) message.setDockableWindow(new ErrorList(message.getView()));
    }
}
