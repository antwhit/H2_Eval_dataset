...
public class TestHelpOpts {
    ...
    void test(String opt) {
        ...
        if (rc != 0) error("Unexpected exit: rc=" + rc);
        String flat = out.replaceAll("\\s+", " ");
        if (!flat.contains("Usage: javah [options] <classes> where [options] include:")) error("expected text not found");
        if (flat.contains("main.opt")) error("key not found in resource bundle: " + flat.replaceAll(".*(main.opt.[^ ]*).*", "$1"));
    }
