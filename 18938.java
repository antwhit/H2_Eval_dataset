...
public class T6420409 {
    ...
    public static void main(String... args) throws IOException {
        ...
        final Iterable<? extends JavaFileObject> compilationUnits = fm.getJavaFileObjectsFromFiles(Arrays.asList(new File(test_src, "T6420409.java")));
        tool.getTask(null, fm, null, Arrays.asList("-proc:none"), null, compilationUnits).call();
    ...
    static void test(Iterable<? extends File> path, File file, Location location) {
        ...
        if (!it.next().equals(file)) throw new AssertionError(file + " not in " + location);
        if (it.hasNext()) throw new AssertionError("Unexpected element in " + location + " : " + it.next());
