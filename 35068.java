...
public class Surface_Plotter implements PlugIn {
    ...
    public ImageProcessor makeSurfacePlot(ImageProcessor ip) {
        ...
        if (oneToOne) polygons = ip.getHeight();
        ...
        y = new int[width + 2];
        double xstart = 10.0;
        if (xinc < 0.0) xstart += Math.abs(xinc) * polygons;
        double ystart = 0.0;
        ...
        for (int i = 0; i < windowWidth * windowHeight; i++) bytes[i] = (byte) ints[i];
    ...
    Polygon makePolygon(double[] profile, double xstart, double ystart) {
        ...
        for (int i = 0; i < width; i++) x[i] = (int) (xstart + i);
        for (int i = 0; i < width; i++) y[i] = (int) (ystart + 255.0 - profile[i]);
