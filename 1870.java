...
public class dnsName {
    ...
    dnsName(String s) {
        ...
        try {
            while (st.hasMoreTokens()) name[labels++] = st.nextToken();
    ...
    dnsName(CountedDataInputStream in, dnsCompression c) throws IOException {
        ...
        while ((len = in.readUnsignedByte()) != 0) {
            if ((len & 0xC0) != 0) {
                ...
                if (name2 == null) name[labels++] = new String("<compressed>"); else {
                    System.arraycopy(name2.name, 0, name, labels, name2.labels);
                    labels += name2.labels;
                }
                break;
            }
    ...
    public short length() {
        short total = 0;
        for (int i = 0; i < labels; i++) total += (name[i].length() + 1);
        return ++total;
    }

    public String toString() {
        StringBuffer sb = new StringBuffer();
        for (int i = 0; i < labels; i++) sb.append(name[i] + ".");
        return sb.toString();
    }
    ...
    public void toBytes(DataOutputStream out) throws IOException {
        for (int i = 0; i < labels; i++) {
            ...
            for (int j = 0; j < name[i].length(); j++) out.writeByte(name[i].charAt(j));
        }
        out.writeByte(0);
    }

    public boolean equals(Object arg) {
        if (arg == null || !(arg instanceof dnsName)) return false;
        dnsName d = (dnsName) arg;
        if (d.labels != labels) return false;
        for (int i = 0; i < labels; i++) {
            if (!d.name[i].equalsIgnoreCase(name[i])) return false;
        }
        return true;
    }
}
