...
public class ReportDateCalculator extends Report {
   ...
   */
    ...
    private static PointInTime getDateFromDateAndAge(PointInTime date, Delta age, int op) {
        ...
        if (calendar != age.getCalendar()) return null;
        int y = date.getYear(), m = date.getMonth(), d = date.getDay();
        if (date.getYear() == PointInTime.UNKNOWN) return null;
    ...
    private static PointInTime normalize(PointInTime pit) {
        ...
        if (month == PointInTime.UNKNOWN) return pit;
        if (month > cal.getMonths()) return normalize(new PointInTime(day, month - cal.getMonths(), year + 1, cal));
        if (month < 0) return normalize(new PointInTime(day, month + cal.getMonths(), year - 1, cal));
        if (day == PointInTime.UNKNOWN) return pit;
        if (day > cal.getDays(month, year)) return normalize(new PointInTime(day - cal.getDays(month, year), month + 1, year, cal));
        if (day < 0) return normalize(new PointInTime(day + cal.getDays(month - 1, year), month - 1, year, cal));
        return pit;
    }
}
