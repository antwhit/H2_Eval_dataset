...
public class TextTool extends Tool {
    ...
    public Shape createShape(WhiteboardContext w, int x, int y, Color c) {
        return new TextShape(x, y, c, w.getText(), w.getFontSize());
    }
    ...
    public Shape shapeFromString(String s) {
        ...
        float fontSize = (ts.length > 3 ? (float) ts[3] : 12f);
        return new TextShape(ts[0], ts[1], new Color(ts[2]), text, fontSize);
    }
    ...
    public static class TextShape extends Shape {
        ...
        public TextShape(int x, int y, Color c, String text, float fontSize) {
            if (text.indexOf("\0") != -1 || text.indexOf("\n") != -1) throw new RuntimeException("Invalid text");
            this.c = c;
            this.x = x;
            this.y = y;
            this.text = text;
        ...
        public String shapeToString() {
            return "t" + x + "," + y + "," + c.getRGB() + "," + (int) fontSize + ";" + hexencode(text);
        }
    }
