...
public class WebsphereCodeGenerator extends CodeGenerator {
    ...
    public void generate(String componentsPath, String[] components) throws Exception {
        ...
        dirBackend.mkdirs();
        String[] argv = { "../OpenXava/xava/default-controllers.xml", backendPath + "/DB.dbxmi" };
        WebsphereDbxmiPG.main(argv);
        for (Iterator it = Mapping.getSchemas().iterator(); it.hasNext(); ) {
            String schema = (String) it.next();
            String[] argvSch = { "../OpenXava/xava/default-controllers.xml", backendPath + "/" + schema + ".schxmi", schema };
            WebsphereSchxmiPG.main(argvSch);
        }
        String[] argvMap = { "../OpenXava/xava/default-controllers.xml", backendPath + "/Map.mapxmi" };
    ...
    protected void generate(MetaComponent component, String componentsPath, String file) throws Exception {
        ...
        String[] argv = { componentsPath + "/" + file, backendPath + "/" + tableId + ".tblxmi", component.getName() };
        WebsphereTblxmiPG.main(argv);
        Iterator itAggregates = component.getMetaAggregatesForCollectionEjbGenerated().iterator();
        while (itAggregates.hasNext()) {
            ...
            String[] argvAg = { componentsPath + "/" + file, backendPath + "/" + aggregateTableId + ".tblxmi", component.getName(), aggregateName };
