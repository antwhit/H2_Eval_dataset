                ...
                for (int i = 0; i < imageReader.getRecord().getImages().length; ++i) {
                    ...
                    try {
                        ...
                        if (irep.equals("RGB") && numBands == 3) {
                            ...
                            ImageIOUtils.showImage(image, file.getName() + "[" + i + "]");
                        } else {
                            for (int j = 0; j < numBands; ++j) {
                                if (bitsPerPixel == 16 || bitsPerPixel == 8 || bitsPerPixel == 32 || bitsPerPixel == 64) {
                                    ImageReadParam readParam = imageReader.getDefaultReadParam();
                                    readParam.setSourceBands(new int[] { j });
                                    BufferedImage image = imageReader.read(i, readParam);
                                    ImageIOUtils.showImage(image, file.getName() + "[" + i + "][" + j + "]");
                                    ImageIO.write(image, "jpg", new FileOutputStream("image" + i + "_" + j + ".jpg"));
                                }
                            }
                        }
                    } catch (Exception e) {
