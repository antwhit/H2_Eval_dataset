public class Parents {
    ...
    public Individual[] tryCrossOver(float crossOverProbability) {
        if (AIMath.getRandomFloat() <= crossOverProbability) return crossOver();
        return asOffsprings();
    }
    ...
    public Individual[] crossOver() {
        ...
        for (int j = 0; j < len; j++) offspring[j].setParents(this);
        return offspring;
    }
    ...
    public Individual[] tryMutation(float mutationProbability) {
        ...
        if (AIMath.getRandomFloat() <= mutationProbability) offsprings[0] = offsprings[0].getMutation();
        if (AIMath.getRandomFloat() <= mutationProbability) offsprings[0] = offsprings[0].getMutation();
        return offsprings;
    }
}
