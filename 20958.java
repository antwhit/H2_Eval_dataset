...
public class TypeParamBounds extends AbstractProcessor {
    ...
    private void doit(Set<? extends TypeElement> annoTypes, RoundEnvironment round) {
        ...
        for (TypeParameterElement tparam : gen.getTypeParameters()) {
            ...
            if (bounds.size() != expected.length) throw new AssertionError();
            int i = 0;
            for (TypeMirror bound : bounds) {
                Name got = types.asElement(bound).getSimpleName();
                String shoulda = expected[i++];
                System.out.println("  " + got);
                if (!got.contentEquals(shoulda)) throw new AssertionError(shoulda);
            }
        }
    }

    static class Gen<T, U extends Object, V extends Number, W extends U, X extends Runnable, Y extends CharSequence & Runnable, Z extends Object & Runnable> {

        static Map<String, String[]> boundNames = new HashMap<String, String[]>();
