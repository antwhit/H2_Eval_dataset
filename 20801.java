...
public class PushbackInputStreamTest extends PushbackInputStream {
    ...
    public PushbackInputStreamTest(InputStream is, int size) {
        super(is, size);
    }
    ...
    public static void main(String[] argv) {
        ...
        {
            PushbackInputStreamTest pist = new PushbackInputStreamTest(new StringBufferInputStream(str), 15);
            boolean passed = true;
            if (pist.pos != pist.buf.length) {
                passed = false;
                System.out.println("The pos variable is wrong.  Expected " + pist.buf.length + " but got " + pist.pos);
            }
            if (pist.buf.length != 15) {
                passed = false;
                System.out.println("The buf.length is wrong.  Expected 15" + " but got " + pist.buf.length);
            }
            if (passed) System.out.println("PASSED: Protected Variables Test"); else System.out.println("FAILED: Protected Variables Test");
        }
        ...
        try {
            ...
            for (int i = 0; i < read_buf1.length; i++) {
                if (read_buf1[i] != read_buf2[i]) passed = false;
            }
            ...
            if (total_read != str.length()) passed = false;
            if (passed) System.out.println("PASSED: Basic Unread Tests"); else System.out.println("FAILED: Basic Unread Tests");
        } catch (IOException e) {
            System.out.println("FAILED: Basic Unread Tests: " + e);
        }
        System.out.println("Test 3: Buffer Overflow Test");
        try {
            PushbackInputStreamTest pist = new PushbackInputStreamTest(new StringBufferInputStream(str), 10);
