...
public class ReportAlmanac extends Report {
   ...
   */
    public void start(PropertyDate[] dates) {
        PointInTime from = new PointInTime(), to = new PointInTime();
        for (int i = 0; i < dates.length; i++) getTimespan(dates[i], from, to);
   ...
   */
    private Iterator getEvents(Gedcom gedcom, Collection indis) {
        PointInTime from = new PointInTime(), to = new PointInTime();
        for (Iterator it = indis.iterator(); it.hasNext(); ) getLifespan((Indi) it.next(), from, to);
   ...
   */
    ...
    private void getTimespan(PropertyDate date, PointInTime from, PointInTime to) {
        ...
        try {
            PointInTime start = date.getStart().getPointInTime(PointInTime.GREGORIAN), end = date.isRange() ? date.getEnd().getPointInTime(PointInTime.GREGORIAN) : start;
            if (!from.isValid() || from.compareTo(start) > 0) from.set(start);
            if (!to.isValid() || to.compareTo(end) < 0) to.set(end);
