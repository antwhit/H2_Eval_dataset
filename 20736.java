...
public class GUI {
    ...
    public static JComboBox getComboBoxFor(int start, int end) {
        ...
        for (int i = start; i <= end; ++i) comboBox.addItem(i);
        return comboBox;
    }
    ...
    public static JScrollPane getScrollPaneFor(Component component, int borderWidth) {
        int policy = ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS;
        JScrollPane scrollPane;
        scrollPane = new JScrollPane(component);
        scrollPane.setMaximumSize(getInfiniteSize());
        scrollPane.setVerticalScrollBarPolicy(policy);
        if (borderWidth > 0) setScrollPaneBorder(scrollPane, borderWidth);
        return scrollPane;
    }

    public static void setScrollPaneBorder(JScrollPane scrollPane, int width) {
        Border border;
        border = BorderFactory.createEmptyBorder(width, width, width, width);
        setScrollPaneBorder(scrollPane, border);
    }

    public static void setScrollPaneBorder(JScrollPane scrollPane, Border border) {
