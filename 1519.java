 ...
 */
class OPT_InsertYieldpointCounters extends OPT_CompilerPhase implements OPT_Operators, VM_Constants, OPT_Constants {
    ...
    */
    public final void perform(OPT_IR ir) {
        if (!ir.method.isInterruptible() || ir.method.getDeclaringClass().isInBootImage() || !VM_Instrumentation.instrumentationEnabled()) return;
        ...
        for (OPT_BasicBlockEnumeration bbe = ir.getBasicBlocks(); bbe.hasMoreElements(); ) {
            ...
            while (i != null && i != bb.lastInstruction()) {
                if (i.operator() == YIELDPOINT_PROLOGUE || i.operator() == YIELDPOINT_EPILOGUE || i.operator() == YIELDPOINT_BACKEDGE) {
                    ...
                    OPT_Instruction counterInst = data.getCounterInstructionForEvent(prefix + ir.method.toString(), incrementValue);
