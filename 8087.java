...
public class Round2Apf implements AnnotationProcessorFactory {
    ...
    private static final Collection<String> supportedAnnotations = unmodifiableCollection(Arrays.asList("Round2"));
    ...
    private static class Round2Ap implements AnnotationProcessor {
        ...
        public void process() {
            ...
            try {
                f.createTextFile(Filer.Location.SOURCE_TREE, "", new File("foo.txt"), null).println("xxyzzy");
                throw new RuntimeException("Duplicate file creation allowed");
            } catch (IOException io) {
            }
            try {
            ...
            try {
                f.createBinaryFile(Filer.Location.CLASS_TREE, "", new File("onezero"));
                throw new RuntimeException("Duplicate file creation allowed");
            } catch (IOException io) {
            }
            try {
