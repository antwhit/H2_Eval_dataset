...
final class OPT_SplitBasicBlock extends OPT_CompilerPhase {
    ...
    final OPT_CompilerPhase newExecution() {
        return this;
    }
    ...
    final void perform(OPT_IR ir) {
        for (OPT_BasicBlockEnumeration e = ir.getBasicBlocks(); e.hasMoreElements(); ) {
            ...
            if (!bb.isEmpty()) {
                while (bb != null) bb = splitEachBlock(bb, ir);
            }
        }
    }
    ...
    final OPT_BasicBlock splitEachBlock(OPT_BasicBlock bb, OPT_IR ir) {
        ...
        for (OPT_Instruction inst = bb.start; inst != bb.end; inst = inst.getNext()) {
            if ((--instCount) == 0) {
                if (inst.isBranch()) return null;
                if (inst.isMove()) {
                    OPT_Instruction next = inst.getNext();
                    if (next != bb.end && next.isImplicitLoad()) inst = next;
                }
